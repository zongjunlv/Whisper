...
def create_chat_completion_old(model, messages, base_url,max_tokens=800,
    ...
    try:
        ...
        for line in response.iter_lines():
            if line:
                ...
                content = decoded_line["data"]["choice"][0]["delta"]["content"]
        decoded_line = response.json()
...
def play_audio(gen_parama, text_url):
    """
    1. 向大模型发送请求，并且得到字符流;
    2. 分别按序播放每句话.
    """
    ...
    model = gen_parama["model"]
    chat_messages = gen_parama["chat_messages"]
    base_url = gen_parama["base_url"]
    max_tokens = gen_parama["max_tokens"]
    temperature = gen_parama["temperature"]
    top_p = gen_parama["top_p"]
    ...
    for sentence in create_chat_completion(model, chat_messages, base_url, max_tokens, temperature, top_p):
        if sentence == "":
            continue
        if stop_tts:
            print("stop_tts = True")
        ...
        if s1 != "":
            ...
            if match_word:
                ...
                if s3 :
                    ...
                    # asyncio.run(main1("chatglm3-6b", chat_messages, base_url, max_tokens, temperature, top_p))
                    gen_param = {
                            "model": "chatglm3-6b",
                    ...
                    play_thread = threading.Thread(target=play_audio,args=(gen_param, text_url))
