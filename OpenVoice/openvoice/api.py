...
class ToneColorConverter(OpenVoiceBaseClass):
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get('enable_watermark', True):
            ...
            try:
                ...
                print("参数 'wav_dir' 不存在")
                raise ke
            self.watermark_model = wavmark.load_model(wav_dir).to(self.device)
        else:
    ...
    def convert(self, audio_src_path: Union[str, np.ndarray], src_se, tgt_se, output_path=None, tau=0.3, message="default", orig_sr="44100"):
        hps = self.hps
        # load audio
        if isinstance(audio_src_path, str):
            audio, sample_rate = librosa.load(audio_src_path, sr=hps.data.sampling_rate)
        else:
            audio = librosa.resample(audio_src_path, orig_sr=orig_sr, target_sr=hps.data.sampling_rate)
