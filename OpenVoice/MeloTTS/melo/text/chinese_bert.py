...
def get_bert_feature(text, word2ph, device=None, model_id='hfl/chinese-roberta-wwm-ext-large'):
    if model_id not in models:
        models[model_id] = AutoModelForMaskedLM.from_pretrained(
            model_id,
            cache_dir=os.path.join(get_root_path(), ".cache"),
            local_files_only=True
        ).to(device)
        tokenizers[model_id] = AutoTokenizer.from_pretrained(model_id, cache_dir=os.path.join(get_root_path(), ".cache"),trust_remote_code=False,local_files_only=True)
